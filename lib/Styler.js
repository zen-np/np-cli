  ...
  Object.keys(COLORS).forEach((function(name) {
    Object.defineProperty(styler, name, {value: (function() {
        var cursor = styler.cursor_,
            base = styler.base_;
        cursor.write(wrapCode(base + COLORS[$traceurRuntime.toProperty(name)]));
        return cursor;
      })});
  }));
};
($traceurRuntime.createClass)(Styler, {
  ...
  color: function(str) {
    ...
    if (str[0] === '#')
      return this.hex(str);
    else if ($traceurRuntime.toProperty(str) in COLORS)
      cursor.write(wrapCode(base + COLORS[$traceurRuntime.toProperty(str)]));
    return cursor;
  },
  bold: function() {
    var cursor = this.cursor_;
    this.bold_ = !this.bold_;
    cursor.write(wrapCode(STYLES.bold[$traceurRuntime.toProperty(this.bold_ ? 0 : 1)]));
    return cursor;
  },
  italic: function() {
    var cursor = this.cursor_;
    this.italic_ = !this.italic_;
    cursor.write(wrapCode(STYLES.italic[$traceurRuntime.toProperty(this.italic_ ? 0 : 1)]));
    return cursor;
  },
  underline: function() {
    var cursor = this.cursor_;
    this.underline_ = !this.underline_;
    cursor.write(wrapCode(STYLES.underline[$traceurRuntime.toProperty(this.underline_ ? 0 : 1)]));
    return cursor;
  },
  inverse: function() {
    var cursor = this.cursor_;
    this.inverse_ = !this.inverse_;
    cursor.write(wrapCode(STYLES.inverse[$traceurRuntime.toProperty(this.inverse_ ? 0 : 1)]));
    return cursor;
  },
  reset: function() {
